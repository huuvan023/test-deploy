...
const Example02 = () => {
  ...
  return (
    <>
      ...
      <div>
        {state1.count2}
        <button onClick={() => dispatch1({ type: 'increment2' })}>+1</button>
        <button onClick={() => dispatch1({ type: 'decrement2' })}>-1</button>
        <button onClick={() => dispatch1({ type: 'set2', count: 0 })}>reset</button>
      </div>
    </>
  );
};
